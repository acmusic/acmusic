...
?>
<?php
	$pcolor = esc_html(get_theme_mod('teacher_profile_clr'));
?>
...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<header class="row entry-header col-md-12">
		<div class="member-bar-lg col-md-12" style="border-color: <?php echo esc_html(get_theme_mod('teacher_profile_clr')); ;?>">
		</div>
		<div>
			<h1 class="col-md-4 entry-title page-entry-title">Teachers</h1>
		</div>
		<div class="">
			<!-- needs to be 55 for production, 68 for staging -->
			<?php $post_id = 55; ?>
	...
	<div class="row member-profile-content">
		<div class="col-xs-12 hidden-sm hidden-md hidden-lg">
			...
			<?php $instrument = get_post_meta( get_the_ID(), 'wpcf-instrument', true ); ?>
			<?php if ($photo != '') : ?>
				<div class="member-photo"><img class="" src="<?php echo esc_url($photo); ?>" alt="<?php echo $name; ?>"></div>
			<?php endif; ?>
			<h1 class="entry-title" style="color: <?php echo esc_html(get_theme_mod('teacher_profile_clr')); ?>"><strong><?php echo $name; ?></strong></h1>
			<h4 style="color: <?php echo esc_html(get_theme_mod('teacher_profile_clr')); ?>"><?php echo $instrument; ?></h4>
			<div class="col-md-12 member-bar-sm" style="border-color: <?php echo esc_html(get_theme_mod('teacher_profile_clr')); ?>;"></div>
		</div>
		<div class="col-sm-7">
